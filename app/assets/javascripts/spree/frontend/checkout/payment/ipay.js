...
Spree.ready(function($) {
  ...
  $('.checkout_form_payment').on('submit', function(e) {
    ...
    if ($('#payment_method_spree_ipay').is(':checked')) {
      ...
      $submitButton.prop('disabled', true).val(Spree.translations.processing || 'Processing...');
      ...
      .done(function(response) {
        if (response.redirect) {
          ...
          showFlash('error', Spree.translables.payment_processing_failed || 'Payment processing failed');
          $submitButton.prop('disabled', false).val(originalText);
        }
      })
      .fail(function(xhr) {
        let errorMessage = Spree.translables.payment_processing_failed || 'Payment processing failed';
        ...
        try {
          ...
          if (response.errors) {
            $.each(response.errors, function(field, messages) {
              ...
              if ($field.length) {
                const $errorDiv = $(`<div class="form-error">${messages.join(', ')}</div>`);
                $field.after($errorDiv).closest('.form-group').addClass('field_with_errors');
              }
            });
          }
        } catch (e) {
